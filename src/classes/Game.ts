...
export class Game {
    ...
    handleEnemyAction() {
        ...
        const { cardAction, turnAction, selectedCard, selectedCardIndices, targetCard, targetCardIndices } = action
        if (turnAction) this.board.processTurnAction(turnAction)
        else if (cardAction) this.board.processCardAction(cardAction, selectedCardIndices, targetCardIndices)
        else throw new Error('invalid action, must specify turnAction or cardAction')
    }
