...
export class Deck {
    ...
    constructor({ unitCards, generalCard }: { unitCards?: Set<UnitCard> | undefined, generalCard?: GeneralCard } = {}) {
        if (!unitCards) unitCards = new Set()
        this.unitCards = new Set(Array.from(unitCards).map(uc => UnitCard.copy(uc)))
