...
export class Board {
    ...
    processAction(action: Action) {
        const { turnAction, cardAction, selectedCardIndices, targetCardIndices } = action
        if (turnAction) {
            this.processTurnAction(turnAction)
        }
        else if (cardAction) {
            this.processCardAction(cardAction, selectedCardIndices, targetCardIndices)
        }
        else throw new Error('unrecognized action type')
    }

    processCardAction(action: CardAction, selectedCardIndices?: CardIndices | undefined, targetCardIndices?: CardIndices | undefined) {
        if (action == CardAction.Draw) {
            if (!selectedCardIndices) throw new Error('selectedCardIndices must be defined for this action!')
    ...
    processCardActionDraw(selectedCardIndices: CardIndices) {
        ...
        const handIndex = selectedCardIndices.getFirstIndex(person, DeckArea.Hand)
        if (handIndex == undefined) throw new Error('hand index must be specified!')
    ...
    calcTargetableIndices(person: Person, handIndex: number, card: UnitCard, action: CardAction) {
        const { deck, opponent, opposingDeck } = this.calcPersonMetadata(person)
